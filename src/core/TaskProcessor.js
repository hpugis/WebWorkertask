...
function getWorkerUrl(moduleID) {
  let url =buildWorkerUrl(moduleID)
...
function getBootstrapperUrl() {
  if (!defined(bootstrapperUrlResult)) {
    bootstrapperUrlResult ="workers/WorkerBootstrapper.js";
  }
  return getWorkerUrl(bootstrapperUrlResult);
}
...
function createWorker(processor) {
  ...
  const bootstrapMessage = {
    loaderConfig: {
      paths: {
        Workers: buildWorkerUrl('workers'),
      },
       baseUrl: buildWorkerUrl.baseUrl,
    },
...
function getWebAssemblyLoaderConfig(processor, wasmOptions) {
  ...
  if (!(typeof WebAssembly !== "undefined")) {
    ...
    config.modulePath = getWorkerUrl(wasmOptions.fallbackModulePath);
    return Promise.resolve(config);
  }

  config.modulePath = getWorkerUrl(wasmOptions.modulePath);
  config.wasmBinaryFile = getWorkerUrl(wasmOptions.wasmBinaryFile);
